...
func runMigrations(db *sql.DB) error {
	...
	// Для Windows: преобразуем путь в формат, понятный для migrate
	// Удаляем букву диска и заменяем обратные слеши на прямые
	migrationPath := strings.TrimPrefix(filepath.ToSlash(absPath), "D:")
	...
	// Используем правильный формат пути для file:// в Windows
